...
class UserController {
  ...
  async getUserByUsername(user: User, username: string) /**:Promise<User> */ {
    if (user.role === Role.ADMIN) {
      ...
      // Check if the logged-in user is an administrator or the requested username matches the logged-in username
      if (user.username !== username) throw new UnauthorizedUserError();
      return this.dao.getUserByUsername(user.username);
    }
  }
  ...
  async deleteUser(user: User, username: string) /**:Promise<Boolean> */ {
    if (user.role === Role.ADMIN) {
      if ((await this.getUserByUsername(user, username)).role === Role.ADMIN)
